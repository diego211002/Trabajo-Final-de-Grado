...
%la Escuela de Imgeniería Mecatrónica
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Tectonic/XeLaTeX also produce PDF directly, but ifpdf's \ifpdf only
% detects pdfTeX. Force the "pdf" branch below and use the correct
% graphics/hyperref driver for whichever engine is actually running.
\ifxetex
  \pdftrue
  \def\HRDriver{xetex}
\else
...
\usepackage[\HRDriver,final]{graphicx}  % for inserting pdf-graphics.
                                        % options final / draft
\ifdraft{%
...
% % (\pdfadjustspacing is a pdfTeX-only primitive; XeTeX doesn't have it)
% %
\ifxetex\else\pdfadjustspacing=1 \fi
% %
\else   % i.e. if not pdf
